...
output "bucket_name" {
  value = coalesce(
    join("", aws_s3_bucket.default.*.bucket),
    join("", data.aws_s3_bucket.default.*.bucket),
  )
...
output "bucket_region" {
  value = coalesce(
    join("", aws_s3_bucket.default.*.region),
    join("", data.aws_s3_bucket.default.*.region),
  )
...
output "bucket_domain_name" {
  value = coalesce(
    ...
    join("", data.aws_s3_bucket.default.*.bucket_domain_name),
  )
...
output "bucket_id" {
  value = coalesce(
    join("", aws_s3_bucket.default.*.id),
    join("", data.aws_s3_bucket.default.*.id),
  )
...
output "bucket_arn" {
  value = coalesce(
    ...
    join("", data.aws_s3_bucket.default.*.arn),
  )
